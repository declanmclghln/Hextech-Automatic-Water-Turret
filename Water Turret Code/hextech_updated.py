...
class HexTech:
    ...
    class StepperMotor:
        ...
        def stop(self, force: bool = True) -> Command:
            return Command(self.board,
                           f"{'+' if force == True else ''}_stepper.{self.pin}_stop")
    ...
    class DCMotor:
        ...
        def move(self, seconds: int) -> Command:
            return Command(self.board, f"dc_move_{seconds}")

        def stop(self, force: bool = True) -> Command:
            return Command(self.board,
                           f"{'+' if force == True else ''}_dc_stop")
    ...
    class BLDCMotor:
        ...
        def move(self, seconds: int) -> Command:
            return Command(self.board, f"bldc_move_{seconds}")

        def stop(self, force: bool = True) -> Command:
            return Command(self.board,
                           f"{'+' if force == True else ''}_bldc_stop")
    ...
    class Axis:
        def __init__(self, board: HexTech, axis: AxisName):
            self.board = board
            # self.pin_number = pin_number
            # # self.pin =  "0" + str(pin_number)
            # self.pin = str(pin_number)
            self.axis = axis
            self.name = axis.value  # Axis Names: X, Y, Z

        def set_speed(self, value: int) -> Command:
            return Command(self.board, f"{self.name}_speed_{value}")

        def set_microsteps(self, microsteps: Microsteps) -> Command:
            return Command(self.board, f"{self.name}_microsteps_{microsteps}")

        def set_acceleration(self, accel: int) -> Command:
            return Command(self.board, f"{self.name}_accel_{accel}")

        def set_current(self, current: int) -> Command:
            return Command(self.board, f"{self.name}_rms_{current}")

        # Moves the Stepper the Specified Number of Steps
        def move(self, steps: int, stealth: bool = True) -> Command:
            return Command(self.board, f"{self.name}_move_{steps}" + ("_0" if not stealth else "_1"))

        # Goes to Specified Position
        def go(self, position: int, stealth: bool = True) -> Command:
            return Command(self.board, f"{self.name}_go_{position}" + ("_0" if not stealth else "_1"))

        # Moves the Stepper to zero (while the limit switch is not pressed)
        def zero(self) -> Command:
            return Command(self.board, f"{self.name}_zero")

        def stop(self, force: bool = True) -> Command:
            return Command(self.board, f"{'+' if force == True else ''}{self.name}_stop")
...
class HexTechMuscle(HexTech):
    def __init__(self, device: str | None = None, verbose: bool = False):
        ...
        self.axisX = self.Axis(self, AxisName.X)
        self.axisY = self.Axis(self, AxisName.Y)
        self.axisZ = self.Axis(self, AxisName.Z)
